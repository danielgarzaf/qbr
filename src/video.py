...
class Calibrator(Webcam):
    ...
    def calibrate(self):
        ...
        cv2.createTrackbar(
            "lowH", "default", self.colors_hsv[self.color][0][0], 180, self.callback
        )
        cv2.createTrackbar(
            "highH", "default", self.colors_hsv[self.color][0][1], 180, self.callback
        )
        cv2.createTrackbar(
            "lowS", "default", self.colors_hsv[self.color][1][0], 255, self.callback
        )
        cv2.createTrackbar(
            "highS", "default", self.colors_hsv[self.color][1][1], 255, self.callback
        )
        cv2.createTrackbar(
            "lowV", "default", self.colors_hsv[self.color][2][0], 255, self.callback
        )
        cv2.createTrackbar(
            "highV", "default", self.colors_hsv[self.color][2][1], 255, self.callback
        )
        ...
        while True:
            ...
            text = "Calibrating {} ({}/{})".format(
                self.colors[self.color_idx].upper(),
                self.color_idx + 1,
                len(self.colors),
            )
            cv2.putText(
    ...
    def check_colors_file(self):
        if not os.path.isfile("./colors.json"):
            raise FileNotFoundError(
                'The file "colors.json" wasn\'t found.\n \
